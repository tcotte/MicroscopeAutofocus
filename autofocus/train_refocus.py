...
from autofocus_dataset import AutofocusDatasetFromMetadata
from logger import WeightandBiaises
from models import RefocusingNetwork
...
parser = argparse.ArgumentParser(
    ...
    description='This program enables to train a model which is able to know from a picture at which '
                'distance_af from the focus the microscope is.',
...
parser.add_argument('-sz', '--img_size', type=int, default=224, required=False,
...
parser.add_argument('-lr', '--learning_rate', type=float, default=0.0001, required=False,
...
# filter images by positive/negative defocus value
X_train_images = filter_by_defocus_sign(list_images=X_train_images, positive_sign=args.positive)
X_test_images = filter_by_defocus_sign(list_images=X_test_images, positive_sign=args.positive)
...
if __name__ == "__main__":
    ...
    conf = {"device": device, "loss": str(criterion), "optimizer": str(optimizer), "lr": args.learning_rate,
            "weight_decay": args.weight_decay, "batch_size": args.batch_size, "nb_epoch": args.epoch,
    ...
    for epoch in range(args.epoch):  # loop over the dataset multiple times
        ...
        with torch.no_grad():
            for data in test_dataloader:
                images, labels = data["X"].float(), data["y"].float()
